...
|-----------|-----------------|
|      양준석      |     준석님 작성1    |
|      박재하      |     이번 두뇌산책 프로젝트를 진행하면서 백엔드, 프론트엔드 개발부터 서버구축과 배포까지 진행하면서 많은 시행착오와 배움을 얻을 수 있었습니다. <br> 백엔드 부분은 멤버 조회 수정과 카카오 로그인, 토큰을 통한 인증을 구현했습니다. 토큰은 엑세스 토큰과 리프레시 토큰을 구현한 후 엑세스 토큰이 만료되면 엑세스 토큰을 재발급하면서 리프레시 토큰도 같이 재발급하는 RTR을 구현했습니다. 리프레시 토큰은 HttpOnly로 설정함으로써 자바스크립트가 쿠키에 접근하지 못하도록 막았습니다.<br> 또한 멀티 디바이스에 대한 로그인을 구현함으로써 다양한 기기로 로그인 할 수 있는 기능을 구현했습니다. 이 과정에서 토큰이 탈취되는 경우를 예방하기 위해 리프레시 토큰은 같은데 user-agent가 다른 경우 토큰이 탈취되었다고 가정하고 리프레시 토큰을 막는 로직을 구현했습니다. 이를 통해 멀티 디바이스 로그인에서 발생할 수 있는 보안 사고에 대해 학습할 수 있었습니다. <br> GlobalException과 BaseException을 구현함으로써 try-catch로 잡는 것이 아닌 BaseException으로 약속된 예외를 던지고 BaseException으로 잡지 못하는 예외는 GlobalException으로 해결함으로써 무중단 배포를 구현했습니다. 시스템 구조는 숨기는 동시에 서버에 상세히 로그를 남길 수 있도록 개발했습니다. 이를 통해 예외처리가 발생했을 때 서버가 중단되면 안되는 것을 인지하고 어떻게 구현해야하는지에 대해 배울 수 있었습니다. <br> SecurityConfig를 구현함으로써 Role을 통해 접근할 수 있는 페이지를 제한할 수 있도록 구현했습니다. <br> 카카오 로그인 부분은 카카오로 로그인을 백엔드 서버에서 요청하면 카카오가 발급하는 토큰을 받은 후 그 토큰을 다시 카카오 서버로 검증하고 검증이 완료되면 카카오ID와 이메일을 받도록 개발했습니다. 이를 통해 사용자 정보는 카카오가 검증 하고 완료 후 카카오 ID와 이메일만 받도록 개발했습니다.<br> 회원 탈퇴 부분도 탈퇴 전 카카오에서 어플에 대한 연결을 해제 한 후 탈퇴로직을 수행하게 함으로써 미아상태가 되는 것을 막았습니다. 또한 30일이 지난후 복구하지 않은 계정은 삭제되도록 스케줄러를 추가함으로써 데이터베이스에 사용하지 않는 데이터를 어떻게 해결해야하는지 알 수 있었습니다. <br> 배포 환경을 구축하는데 있어 생각만 해봤던 구성을 실제 구현함으로써 다양한 문제에 직면했고 이를 해결하는 과정을 통해 많은 내용을 공부할 수 있었습니다. 사용자는 ALB를 거쳐서만 API를 호출 하고 80과 8080으로 접근할 수 있도록 구현했습니다. ALB에는 http로 사용자가 접근하면 자동으로  https로 리다이렉트 되도록 구현했습니다. 이를 AWS Certificate Manager에서 SSL/TLS 인증서를 발급받은 후 ALB에 등록함으로 써 도메인에 대한 신뢰성을 확보할 수 있었습니다. <br> 또한 데이터베이스 서버는 EC2에서만 RDS로 접근할 수 있도록 보안그룹을 생성했습니다. 환경변수는 SSM ParameterStore에 등록한 후 EC2가 직접 가져가도록 기능을 구현했습니다. 개발자는 ALB가 아닌 EC2 서버에 직접 접근할 수 있도록 SSH에 IP 화이트리스트를 등록하고 키를 등록해서 키를 통해서만 접근할 수 있도록 구현했습니다. 프론트 부분은 S3에 패키징한 파일을 등록하고 CloudFront로 배포하도록 설정했습니다. CloudFront는 엣지캐싱을 통해 전세계에서 빠르게 접근할 수 있기에 기능을 채택했습니다. 또한 S3에 패키징 된 파일들을 CloudFront 통해서만 S3에 접근할 수 있도록 OAC를 설정할 수 있기 때문에 사용자가 S3 엔드포인트 URL에 직접 접근할 수 없고 CloudFront를 통해서만 접근할 수 있습니다. EC2나 RDS의 사용량과 백엔드 서버 로그는 CloudWatch를 통해 조회 할 수 있도록 설정했습니다. <br>이 과정을 통해서 AWS를 통한 배포에 대해 배울 수 있었습니다. 그리고 비용이 충분하다면 ALB를 통해 둘 이상의 가용영역에서 EC2를 자동으로 스케일되게 설정하고 RDS도 개발서버, 배포서버로 구성하거나 Aurora를 통해서 여러 리전에 배포함으로써 안정성을 높여보고 싶습니다. 또한 EC2와 RDS를 VPC 내부로 숨기고 NAT GATEWAY를 통해서만 접근할 수 있도록 구성해보고 싶습니다.     |
|      모희주      |     희주님 작성     |
|      윤준상      |     준상님 작성     |
