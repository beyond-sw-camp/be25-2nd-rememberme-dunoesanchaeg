...
|-----------|-----------------|
|      양준석      |     준석님 작성1    |
|      박재하      |     백엔드 부분은 멤버 조회 수정과 카카오 로그인, 토큰을 통한 인증을 구현했습니다. <br> 토큰은 엑세스 토큰과 리프레시 토큰을 구현한 후 엑세스 토큰이 만료되면 엑세스 토큰을 재발급하면서 리프레시 토큰도 같이 재발급하는 RTR을 구현했습니다. 또한 멀티 디바이스에 대한 로그인을 구현함으로써 다양한 기기로 로그인 할 수 있는 기능을 구현했습니다. 이 과정에서 토큰이 탈취되는 경우를 예방하기 위해 리프레시 토큰은 같은데 user-agent가 다른 경우 토큰이 탈취되었다고 가정하고 리프레시 토큰을 막는 로직을 구현했습니다. 이를 통해 멀티 디바이스 로그인에서 발생할 수 있는 보안 사고에 대해 학습할 수 있었습니다. 실제 로그인 정보를 구현하는 과정을 수행하면서 사용자의 정보를 정말 신중하게 다뤄야하는 것을 배울 수 있었습니다.<br> 배포 환경을 구축하는데 있어 생각만 해봤던 구성을 실제 구현함으로써 다양한 문제에 직면했고 이를 해결하는 과정을 통해 많은 내용을 공부할 수 있었습니다. ALB와 EC2, RDS를 설정하고 프론트는 CloudFront와 S3를 설정해봤습니다. 이를 통해 머리로만 생각해봤던 상황을 구현해보는 과정에서 많은 내용을 배울 수 있었습니다.     |
|      모희주      |     희주님 작성     |
|      윤준상      |     준상님 작성     |
