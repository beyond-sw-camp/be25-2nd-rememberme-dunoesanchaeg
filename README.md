...
|-----------|-----------------|
|      양준석      |     준석님 작성1    |
|      박재하      |     백엔드 부분은 멤버 조회 수정과 카카오 로그인, 토큰을 통한 인증을 구현했다. <br> 토큰은 엑세스 토큰과 리프레시 토큰을 구현한 후 엑세스 토큰이 만료되면 엑세스 토큰을 재발급하면서 리프레시 토큰도 같이 재발급하는 RTR을 구현했다. 또한 멀티 디바이스에 대한 로그인을 구현함으로써 다양한 기기로 로그인 할 수 있는 기능을 구현했다. 이 과정에서 토큰이 탈취되는 경우를 예방하기 위해 리프레시 토큰은 같은데 user-agent가 다른 경우 토큰이 탈취되었다고 가정하고 리프레시 토큰을 막는 로직을 구현했다. 이를 통해 멀티 디바이스 로그인에서 발생할 수 있는 보안 사고에 대해 학습할 수 있었다. 실제 로그인 정보를 구현하는 과정을 수행하면서 사용자의 정보를 정말 신중하게 다뤄야하는 것을 배울 수 있었다.<br> 배포 환경을 구축하는데 있어 생각만 해봤던 구성을 실제 구현함으로써 다양한 문제에 직면했고 이를 해결하는 과정을 통해 많은 내용을 공부할 수 있었습니다. ALB와 EC2, RDS를 설정하고 프론트는 CloudFront와 S3를 설정해봤다. 이를 통해 머리로만 생각해봤던 상황을 구현해보는 과정에서 많은 내용을 배울 수 있었다.
     |
|      모희주      |     희주님 작성     |
|      윤준상      |     준상님 작성     |
...
|      조하은      |     이번 프로젝트를 통해 예외처리를 체계적으로 설계하는 것이 얼마나 중요한지 직접 느낄 수 있었다. 사용자 입장에서 서비스를 사용하는 상황을 고려하다 보니 입력값 검증, 중복 처리, 잘못된 요청 대응 등 다양한 예외 케이스를 처리하게 되었고, 그 과정에서 구현 범위가 과도하게 확장되는 어려움도 있었다. 이를 위해 BaseException을 기반으로 커스텀 예외를 정의하고, Global Exception Handler를 통해 예외를 일관된 방식으로 처리했으며, 팀원들과의 지속적인 피드백을 통해 우선순위를 조정하며 적절한 수준으로 기능을 완성할 수 있었다. 또한 ApiResponse 객체로 API 응답 형식을 통일하면서, 프론트엔드와의 협업 시 데이터 구조를 보다 명확하게 전달할 수 있었다.
프론트엔드 연동 과정에서는 DTO 구조, 데이터 타입, null 처리 등을 맞추는 작업이 예상보다 까다로웠다. 이 경험을 통해 협업할 때 명확한 규칙을 사전에 정의하는 것이 얼마나 중요한지 실감했다. 아울러 application-local.yml을 활용해 환경별 설정을 분리하면서, 로컬과 서버 환경을 각각 고려한 개발 방식도 익힐 수 있었다.
Git을 활용한 협업에서는 브랜치 생성과 병합 과정에서 직접 충돌을 해결하며 버전 관리의 중요성을 체감했다. 특히 CLI 환경에서 git add, commit, push 등의 명령어를 직접 입력하다 보니 Git의 동작 원리가 훨씬 잘 이해되었고, 이전보다 한층 능숙하게 협업을 진행할 수 있게 되었다.
이번 프로젝트를 거치면서 백엔드 개발 전반에 대한 이해가 깊어졌다.     |
