...
public class CognitiveGameController {
    ...
    // 2. 게임 결과 저장
    @PostMapping("/result")
    public ApiResponse<GameFinishedResponse> saveGameResult(@Valid @RequestBody AnswerSubmitRequest request) {
        Long memberId = SecurityUtil.getCurrentMemberId();

        GameFinishedResponse response = cognitiveGameService.saveGameResult(memberId, request);

        return ApiResponse.success(200, "미니게임 결과 저장 성공", response);
    }
}
